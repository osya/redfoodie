    ...
    public class HomeControllerTest
    {
        ...
        private static ApplicationDbContext Db
        {
            get
            {
                ...
                mockUsers.As<IQueryable<ApplicationUser>>()
                    .Setup(m => m.Expression)
                    .Returns(usersQueriableList.Expression);
                mockUsers.As<IQueryable<ApplicationUser>>()
                    .Setup(m => m.ElementType)
                    .Returns(usersQueriableList.ElementType);
                mockUsers.As<IQueryable<ApplicationUser>>()
                    .Setup(m => m.GetEnumerator())
                    .Returns(usersQueriableList.GetEnumerator());
                ...
                mockCities.As<IQueryable<City>>()
                    .Setup(m => m.GetEnumerator())
                    .Returns(citiesQueriableList.GetEnumerator());
                mockCities.Setup(m => m.Find(It.IsAny<object[]>()))
                    .Returns<object[]>(ids => Cities.FirstOrDefault(d => Equals(d.Id, ids[0])));
                ...
                mockPlaces.As<IQueryable<Place>>()
                    .Setup(m => m.ElementType)
                    .Returns(placesQueriableList.ElementType);
                mockPlaces.As<IQueryable<Place>>()
                    .Setup(m => m.GetEnumerator())
                    .Returns(placesQueriableList.GetEnumerator());

                // Create a list of cuisines
                var cuisinesQueriableList =
                    Commons.CuisineList.Select(c => new Cuisine {Id = Commons.GetStringId(c), Name = c})
                        .AsQueryable();
                ...
                mockCuisines.As<IQueryable<Cuisine>>()
                    .Setup(m => m.Expression)
                    .Returns(cuisinesQueriableList.Expression);
                mockCuisines.As<IQueryable<Cuisine>>()
                    .Setup(m => m.ElementType)
                    .Returns(cuisinesQueriableList.ElementType);
                mockCuisines.As<IQueryable<Cuisine>>()
                    .Setup(m => m.GetEnumerator())
                    .Returns(cuisinesQueriableList.GetEnumerator());

                // Create a list of restaurant groups
                var restaurantGroupsQueriableList =
                    Commons.RestaurantGroups.Select(
                        g =>
                            new RestaurantGroup
                            {
                                Id = Commons.GetStringId(g.Key),
                                Name = g.Key,
                                ImageFileName = g.Value
                            }).AsQueryable();
                ...
                mockRestaurantGroups.As<IQueryable<RestaurantGroup>>()
                    .Setup(m => m.Expression)
                    .Returns(restaurantGroupsQueriableList.Expression);
                mockRestaurantGroups.As<IQueryable<RestaurantGroup>>()
                    .Setup(m => m.ElementType)
                    .Returns(restaurantGroupsQueriableList.ElementType);
                mockRestaurantGroups.As<IQueryable<RestaurantGroup>>()
                    .Setup(m => m.GetEnumerator())
                    .Returns(restaurantGroupsQueriableList.GetEnumerator());

                return new ApplicationDbContext
                {
                    Users = mockUsers.Object,
                    Cities = mockCities.Object,
                    Places = mockPlaces.Object,
                    Cuisines = mockCuisines.Object,
                    RestaurantGroups = mockRestaurantGroups.Object
                };
            }
        }
        ...
        /// <summary>
        /// Simple test method for testing AppHarbor testing capability
        /// </summary>
        [TestMethod]
        public void SimpleFakeTest()
        {
